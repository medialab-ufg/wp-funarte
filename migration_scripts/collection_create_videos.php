...
$col_name = 'Videos_Script';
$col_desc = 'Acervo de vídeos produzidos pela Funarte.';
$path_to_metadata = 'metadata_videos.csv';
...
$collection->set_name($col_name);
$collection->set_status('publish');
$collection->set_description($col_desc);

$cont = 0;
if ($collection->validate()) {
	$insertedCollection = $collectionsRepo->insert($collection);

	if (($handle = fopen($path_to_metadata, "r")) == TRUE) {
				...
				if (trim($data[3]) == 'Description' OR trim($data[3]) == 'Title'){
					...
					if (trim($data[2]) == 'Tainacan\Metadata_Types\Taxonomy'){
						...
						if (trim($data[3]) == 'Entidade'){
							...
							$metadata_type_options = ['multiple'=>'yes', 'repeated'=>'yes', 'input_type' => 'tainacan-taxonomy-tag-input'];
