...
class LinkRelacionado {
	use Singleton;

	private $POST_TYPE = "link-relacionado";
	...
	public function register_post_type() {
		$POST_TYPE_NAME_PLURAL = "Links relacionados";
		$POST_TYPE_NAME_SINGULAR = "Link relacionado";
	...
	public function add_custom_box() {
		add_meta_box('link_relacionado_url',__('URL'),
					array(&$this, 'link_relacionado_custom_box'), $this->POST_TYPE, 'advanced', 'high');
	}
	...
	public function save_destaque_home_custom_box($post_id) {
		...
		// Verifica o nonce
		if (!wp_verify_nonce($_POST['url_custom_box'], __FILE__)) {
			return $post_id;
		}
		...
		$fields = array('url');
		$data = $_POST['linkrelacionado'];
		foreach ($data as $field => $value) {
			if (!in_array($field, $fields)) {
				continue;
			}
			update_post_meta($post_id, 'linkrelacionado-' . $field, $value);
		}
	}

	public function link_relacionado_custom_box() {
		global $post;
		$nonce = wp_create_nonce(__FILE__);
		$url = get_post_meta($post->ID, 'linkrelacionado-url', true);
		$THEME_FOLDER = get_template_directory();
		$DS = DIRECTORY_SEPARATOR;
		$META_FOLDER = $THEME_FOLDER . $DS . 'inc' . $DS . 'post_types' . $DS . 'link_relacionado' . $DS;
		require_once($META_FOLDER . 'metabox-url.php');
	}
