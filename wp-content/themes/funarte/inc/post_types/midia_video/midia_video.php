...
class MidiaVideo {
	...
	public function register_post_type() {
		$POST_TYPE_NAME_PLURAL = "Mídias Vídeos";
		$POST_TYPE_NAME_SINGULAR = "Mídia Vídeo";
	...
	public function add_custom_box() {
		add_meta_box('video_url',__('URL'),
					array(&$this, 'video_ulr_custom_box'), $this->POST_TYPE, 'advanced', 'high');
	}

	public function video_ulr_custom_box() {
		global $post;
		$nonce = wp_create_nonce(__FILE__);
		$url = get_post_meta($post->ID, 'midia-video-url', true);
		$THEME_FOLDER = get_template_directory();
		$DS = DIRECTORY_SEPARATOR;
		$META_FOLDER = $THEME_FOLDER . $DS . 'inc' . $DS . 'post_types' . $DS . 'midia_video' . $DS;
		require_once($META_FOLDER . 'metabox-video-url.php');
	}
	...
	public function save_custom_box($post_id) {
		...
		$this->save_video_url_box($post_id);
	}

	public function save_video_url_box($post_id) {
		if (empty($_POST)) {
			return $post_id;
		}
		// Verifica o nonce
		if (!wp_verify_nonce($_POST['video_url_custom_box'], __FILE__)) {
			return $post_id;
		}
		...
		$url = $_POST['video_url'];
		update_post_meta($post_id, 'midia-video-url', $url);
	}

}

MidiaVideo::get_instance();